...
async function HandleArgv() {
  ...
  program.command("clear_history").action(() => {
    console.log("Cleared History");
    history.length = 0;
    fs.writeFileSync("./config.json", JSON.stringify(config, null, 2), "utf-8");
  });
  program.command("clear_marked").action(() => {
      ...
      source: async (input, { signal }) => {
        ...
        try {
          ...
          return data.data.map((mg) => ({
            name:
              mg.attributes.title.en || Object.values(mg.attributes.title)[0],
            value:
              mg.attributes.title.en || Object.values(mg.attributes.title)[0],
          }));
        } catch (error) {
...
async function DownloadManga(
  ...
  console.log(
    "[INFO]:",
    chalk.blue("Downloading", manga_title, "in", folder_path),
  );
  console.log("");
...
async function DownloadMangaQuery(manga_title) {
  ...
  if (!answer) {
    ...
    if (!fs.existsSync(fullPath)) {
      ...
      const answerDeleteDir = await confirm({
        message: `A folder for ${manga_title} already exists. Should I delete it? `,
        theme: ErrorPrompt,
      });
      if (!answerDeleteDir) {
        await SearchManga();
